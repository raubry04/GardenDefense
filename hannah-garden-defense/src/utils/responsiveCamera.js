...
import { fitDesignInScreenRect, DESIGN } from './battleLayout.js';
...
export function fillGrassUnderlay(scene, view, depth = -10) {
  ...
  const startX = Math.floor(view.x / tileSize) * tileSize;
  const startY = Math.floor(view.y / tileSize) * tileSize;
  const endX = Math.ceil(view.right / tileSize) * tileSize;
  const endY = Math.ceil(view.bottom / tileSize) * tileSize;
...
export function refillSceneGrass(scene, view, depth = -10) {
  if (scene._grassObjs) scene._grassObjs.forEach((o) => o.destroy());
  scene._grassObjs = fillGrassUnderlay(scene, view, depth);
}
